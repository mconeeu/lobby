...
public class BankInfosInventory1 extends CoreInventory {

    public BankInfosInventory1(Player p) {
        ...
        } else if (!lp.hasLobbyItem(LobbyItem.NORMAL_FILES_2)) {
            setItem(InventorySlot.ROW_2_SLOT_5, LobbyItem.NORMAL_FILES_2.getItemStack(), e -> {
                lp.addLobbyItem(LobbyItem.NORMAL_FILES_2);
                Bukkit.getScheduler().runTaskLaterAsynchronously(LobbyPlugin.getInstance(), () -> {
                    if (lp.hasLobbyItem(LobbyItem.NORMAL_FILES_1) && lp.hasLobbyItem(LobbyItem.NORMAL_FILES_3)) {
