...
public class BankInfosInventory2 extends CoreInventory {

    public BankInfosInventory2(Player p) {
        ...
        } else if (!lp.hasLobbyItem(LobbyItem.NORMAL_FILES_3)) {
            setItem(InventorySlot.ROW_2_SLOT_5, LobbyItem.NORMAL_FILES_3.getItemStack(), e -> {
                lp.addLobbyItem(LobbyItem.NORMAL_FILES_3);
                Bukkit.getScheduler().runTaskLaterAsynchronously(LobbyPlugin.getInstance(), () -> {
                    if (lp.hasLobbyItem(LobbyItem.NORMAL_FILES_1) && lp.hasLobbyItem(LobbyItem.NORMAL_FILES_2)) {
                        p.sendMessage("§8[§7§l!§8] §cKnopf im Ohr §8» §fJohn§8 |§7 Super du hast noch mehr Daten gefunden komme jetzt zurück!");
                    } else {
                        p.sendMessage("§8[§7§l!§8] §cKnopf im Ohr §8» §fJohn§8 |§7 So viele Daten ... Suche weiter oder komm zurück!");
                    }
                }, 15L);
                p.closeInventory();
            });
