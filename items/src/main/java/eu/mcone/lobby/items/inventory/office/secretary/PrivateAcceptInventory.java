...
public class PrivateAcceptInventory extends CoreInventory {
    ...
    private static final CoreAnvilInventory ANVIL_INVENTORY = CoreSystem.getInstance().createAnvilInventory(event -> {
        ...
        if (target != null && !target.equals(event.getPlayer())) {
            ...
            if (event.getSlot().equals(AnvilSlot.OUTPUT)) {
                ...
                SecretaryInventory.isInviting.add(player);
    ...
    public PrivateAcceptInventory(Player player) {
        super("§8» §b§lSekretärin", player, InventorySlot.ROW_3, InventoryOption.FILL_EMPTY_SLOTS);


        setItem(InventorySlot.ROW_2_SLOT_5, new ItemBuilder(Material.STAINED_GLASS_PANE, 1, 13).displayName("§aBestätigen")
                .lore("§fWenn du fortfährst verlässt du die", "§fPrivate Lobby und kommst", "§fvorerst nicht mehr rein!")
                .create(), e -> {
