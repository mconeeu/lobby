...
public class GangInventory extends CoreInventory {
    ...
    public GangInventory(Player p) {
        super(LobbyPlugin.getInstance().getLobbyPlayer(p.getUniqueId()).isInGang() ? "§2§l"+LobbyPlugin.getInstance().getLobbyPlayer(p.getUniqueId()).getGang().getName() : "§2§lGang gründen", p, InventorySlot.ROW_6);
