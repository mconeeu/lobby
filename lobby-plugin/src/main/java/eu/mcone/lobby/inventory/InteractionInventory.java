...
public class InteractionInventory extends CoreInventory {
    ...
    public InteractionInventory(Player p, Player clicked) {
        ...
        CoreSystem.getInstance().getMySQL(1).select("SELECT status, coins, onlinetime FROM userinfo WHERE uuid='" + clicked.getUniqueId().toString() + "'", rs -> {
            try {
                if (rs.next()) {
                    ...
                    setItem(20, ItemBuilder.createSkullItemFromURL("http://textures.minecraft.net/texture/6f74f58f541342393b3b16787dd051dfacec8cb5cd3229c61e5f73d63947ad", 1).displayName("§7Online-Profil Ansehen").create(), () -> {
                        TextComponent tc0 = new TextComponent(TextComponent.fromLegacyText(LobbyPlugin.getInstance().getConf().getConfigValue("System-Prefix") + "§2Das Profil von " + clicked.getName() + " findest du "));
                    ...
                    CoreSystem.getInstance().getMySQL(1).select("SELECT uuid FROM `bungeesystem_friends` WHERE `uuid`='"+player.getUniqueId()+"' AND `target`='"+clicked.getUniqueId()+"' AND `key`='friend';", rs1 -> {
                        try {
                            if (rs1.next()) {
